...
class RegionPipeline:
    ...
    def predict_next(self, current_data):
        """Predict next timestep using last 24 hours of data"""
        # Preprocess and validate
        preprocessed = self.preprocess_new_data(current_data)
        if len(preprocessed) < 24:
            raise ValueError(f"Need 24 timesteps, got {len(preprocessed)}")
        
        # Use last 24 timesteps
        sequence = preprocessed[-24:]
        sequence_tensor = torch.FloatTensor([sequence]).to(self.device)
        
        # Get LSTM features
        with torch.no_grad():
            self.lstm_model.eval()
            lstm_out = self.lstm_model(sequence_tensor).cpu().numpy()
        ...
        for target in self.target_variables:
            pred = self.xgb_models[target].predict(combined_features)
            predictions[target] = self.target_scalers[target].inverse_transform(
                pred.reshape(-1, 1)).flatten()[0]
        return predictions
